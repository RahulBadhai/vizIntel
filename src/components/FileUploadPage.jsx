...
const FileUploadPage = () => {
  ...
  const updateEditColumnName = (index, newName) => {
    ...
    updatedColumns[index] = trimmedName || oldName;
  ...
  const updateGraphs = (data) => {
    ...
    const keyColumn = data[0] ? Object.keys(data[0])[0] : "key";
  ...
  return (
    <div className="min-h-screen bg-black text-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        >
          <h2 className="text-3xl font-bold mb-4 text-orange-500">File Upload</h2>
          {errorMessage && (
            <div className="bg-red-600 text-white p-3 rounded-lg mb-4">{errorMessage}</div>
          )}

          <form onSubmit={handleFileUpload} className="mb-4">
            <label className="flex items-center border border-gray-700 bg-gray-800 text-gray-300 p-2 rounded-lg cursor-pointer glow-hover">
            ...
            <button
              type="submit"
              className="p-2 bg-orange-500 hover:bg-orange-600 text-black rounded-lg glow-hover mt-2"
            >
              Upload Excel
            </button>
          </form>
